...
class EditDeploymentVersion extends Component {
  ...
  componentDidMount() {
    ...
    const {AppState, AdminEditVersionStore} = this.props;
    let organizationId = AppState.menuType.id;
    AdminEditVersionStore.getVersionById(organizationId, deploymentId, versionId).then(data => {
      if (data) {
  ...
  handleSubmit() {
    ...
    if (version && this.state.config && this.state.deploy) {
      ...
      AdminEditVersionStore.updateDeploymentVersion(organizationId, deploymentId, versionId, data).then(res => {
        if (res) {
          this.setState({
            submitting: false,
          });
          message.success(HAP.getMessage("修改成功", "Success"));
          this.linkToChange(`/iam/deploymentAdmin/${deploymentId}/version`);
        }
      }).catch(error => {
    ...
    return (
      <div>
        ...
        <div style={{width: this.state.windowWidth + 'px'}}>
          ...
          <p style={{color: 'red', fontSize: '12px', marginLeft: '5px', minHeight: '20px'}}>{this.state.configError}</p>
        ...
        <div style={{width: this.state.windowWidth + 'px'}}>
          ...
          <p style={{color: 'red', fontSize: '12px', marginLeft: '5px', minHeight: '20px'}}>{this.state.deployError}</p>
          <CodeMirror ref="deploy" value={this.state.deploy} onChange={this.updateDeploy} options={options}
                      autoFocus={false}/>
        </div>

        <p style={{color: 'red', fontSize: '12px', marginLeft: '5px', minHeight: '20px'}}>{this.state.error||''}</p>
