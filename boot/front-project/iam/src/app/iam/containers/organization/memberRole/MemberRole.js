...
@inject("AppState")
@observer
class OrganizationMemberRole extends Component {
  ...
  //加载角色列表数据
  loadRoles = () => {
    const { AppState, OrganizationMemberRoleStore } = this.props;
    const menuType = AppState.currentMenuType;
    let organizationId = menuType.id;
    OrganizationMemberRoleStore.loadRoles(organizationId);
  ...
  loadMemberRoles = () => {
    ...
    const menuType = AppState.currentMenuType;
    let organizationId = menuType.id;
    OrganizationMemberRoleStore.loadMemberRoles(organizationId);
  ...
  loadUserData = () => {
    ...
    const menuType = AppState.currentMenuType;
    let organizationId = menuType.id;
    OrganizationMemberRoleStore.loadUserData(organizationId);
  ...
  handleAssignShow = (recode) => {
    const { OrganizationMemberRoleStore } = this.props;
    const isShow = OrganizationMemberRoleStore.getIsShow;
    if (isShow) {
      return;
    }
    if (recode) {
      ...
      OrganizationMemberRoleStore.setShow(true);
      this.setState({
        roleKeys: tmp,
  ...
  handleDelete = () => {
    ...
    roles.map(item => {
      OrganizationMemberRoleStore.handleDelete(item.id, success, total, this.loadMemberRoles());
  ...
  handleBatchDelete = () => {
    ...
    memberRoleIds.map(id => {
      OrganizationMemberRoleStore.handleBatchDelete(id, success, total, this.loadMemberRoles());
  ...
  handleClose = () => {
    const { OrganizationMemberRoleStore } = this.props;
    const isShow = OrganizationMemberRoleStore.setShow(false);
  };
  ...
  handleRoleSave = (data) => {
    ...
    OrganizationMemberRoleStore.handleRoleSave(role, this.loadMemberRoles());
  ...
  handleAddOk = () => {
    ...
    const menuType = AppState.currentMenuType;
    let organizationId = menuType.id;
    const userData = OrganizationMemberRoleStore.getuserData;
    ...
    if (tmp.length > 0) {
      ...
      data.map(item => {
        OrganizationMemberRoleStore.handleAddOk(organizationId, item, this.loadMemberRoles(), this.handleAddCancel());
      });
      this.loadMemberRoles();
    } else {
  ...
  handleChange = (value) => {
    ...
    if (value == "user") {
      OrganizationMemberRoleStore.setIsUser(true);
      OrganizationMemberRoleStore.setShow(false);
    } else if (value == "role") {
      OrganizationMemberRoleStore.setIsUser(false);
      OrganizationMemberRoleStore.setShow(false);
    } else {
      OrganizationMemberRoleStore.setIsUser(true);
      OrganizationMemberRoleStore.setShow(false);
  ...
  handleSearch = (value) => {
    if (value) {
      ...
      const menuType = AppState.currentMenuType;
      let organizationId = menuType.id;
      OrganizationMemberRoleStore.handleSearch(organizationId, value);
      // fetch(`${USER_API_HOST}/organization/${organizationId}/memberRoles`, {
      //   headers: HAP.getHeader()
      // }).then(HAP.convertResponse(this)).then((data) => {
      //   if (data) {
  ...
  render() {
    ...
    const { OrganizationMemberRoleStore } = this.props;
    const memberRole = OrganizationMemberRoleStore.getMemberRole;
    const isLoading = OrganizationMemberRoleStore.getIsLoading;
    const isShow = OrganizationMemberRoleStore.getIsShow;
    const roleData = OrganizationMemberRoleStore.getRoleData;
    const isUser = OrganizationMemberRoleStore.getIsUser;
    const userData = OrganizationMemberRoleStore.getuserData;
    //定义加载样式
    ...
    //定义表格的列
    const columns = [{
      title: HAP.languageChange('organizationMemberRole.type'),
      dataIndex: 'memberType',
      key: 'memberType',
      render: (text, record) => (
        <div>
          <Tooltip placement="right" title={text == "user" ? HAP.languageChange("organizationMemberRole.user") : HAP.languageChange("organizationMemberRole.organization")}>
            {text == "user" ? <Icon type="user" /> : <Icon type="database" />}
          </Tooltip>
        </div>
      )
    }, {
      title: HAP.languageChange('organizationMemberRole.member'),
      dataIndex: 'userName',
      key: 'userName',
      render: (item, record) => (
        <div>
      ...
      title: HAP.languageChange('organizationMemberRole.role'),
      dataIndex: 'roles',
      key: 'roles',
      render: (text, record) => (
        ...
        <RoleCas treeData={treeData}
          ...
          onClicks={this.handleAssignShow.bind(this, record)} />
      )
    }, {
      title: <div style={{ textAlign: "center" }}>
        {HAP.languageChange('organizationMemberRole.operate')}
      </div>,
      className: "operateIcons",
      key: 'action',
      render: (text, record) => (
        <div>
          <Tooltip title={HAP.languageChange('organizationMemberRole.cancel')} placement="bottom" getTooltipContainer={(that) => that}>
    ...
    return (
      <div style={style.container}>
        <div style={style.top}>
          <PageHeader title={HAP.languageChange('organizationMemberRole.MemberRoleOrganization')}>
            <Button className="header-btn" ghost={true} style={PageHeadStyle.leftBtn}
              icon="user-add" onClick={this.handleAddModalShow}>{HAP.languageChange('organizationMemberRole.add')}</Button>
            <Modal
            ...
            >
              ...
              <Row>
                <Col span={2}>
                  <label>{HAP.languageChange("organizationMemberRole.user")}:</label>
                </Col>
                <Col span={8}>
                  <Mention
                ...
                <Col span={4} offset={2}>
                  <label>{HAP.languageChange("organizationMemberRole.role")}:</label>
                </Col>
                <Col span={7}>
            ...
            <Button className="header-btn" ghost={true} style={PageHeadStyle.leftBtn} onClick={this.handleBatchRemoveOpen}
              icon="user-delete">{HAP.languageChange('organizationMemberRole.cancel')}</Button>
            <Button className="header-btn" ghost={true} onClick={() => {
              this.loadMemberRoles()
            }} style={PageHeadStyle.leftBtn} icon="reload">{HAP.languageChange("organizationMemberRole.refresh")}</Button>
          </PageHeader>
        </div>
          ...
          <div style={style.bottom}>
            <div style={style.left}>
              ...
              <div style={{ margin: 20, }}>
                ...
                />
                <span style={{ marginLeft: 15, }}>{HAP.languageChange('organizationMemberRole.lookMethod')}</span>
                <Select defaultValue="user" style={{ width: 120, marginLeft: 16 }} onChange={this.handleChange}>
                  <Option value="user">{HAP.languageChange("organizationMemberRole.member")}</Option>
                  <Option value="role">{HAP.languageChange("organizationMemberRole.role")}</Option>
                </Select>
              </div>
...
export default withRouter(OrganizationMemberRole);
