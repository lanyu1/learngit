...
class EditClient extends Component {
  ...
  componentDidMount() {
    ...
    AdminClientStore.getClientById(this.state.id).then(data=>{
      this.setState({
        client:data,
      });
    }).catch(error=>{
      message.error(HAP.getMessage("数据加载失败","The data load failure"))
    })
  }
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFieldsAndScroll((err, data) => {
      if (!err) {
        ...
        console.log("修改",data);
        AdminClientStore.updateClient(this.state.id,
  ...
  render() {
    ...
    return (
        <div>
          <PageHeader title={HAP.languageChange("client.edit")} backPath="/iam/admin-client"/>
          {client ? (
