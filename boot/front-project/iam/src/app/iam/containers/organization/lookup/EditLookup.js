...
import {Form, Input, Button, Row, Col, Modal, Table, Tooltip, message,Icon} from 'antd';
import Remove from '../../../components/Remove';
import PageHeader from '../../../components/PageHeader';

@inject("AppState")
@observer
class EditLookup extends Component {
    constructor(props){
        super(props);
        this.state = {
            submiting: false,
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      if (!err) {
        ...
        this.props.LookupStore.updateLookupById(this.state.organizationId, this.state.id, data).then(()=>{
    ...
    componentDidMount(){
        this.loadEditLookup(this.state.organizationId, this.state.id);
    }

    loadEditLookup(organizationId, lookupId){
        this.props.LookupStore.getLookupById(organizationId, lookupId).then(data=>{
            this.setState({
                datas: data.values,
                code: data.code,
                description: data.description,
                objectVersionNumber: data.objectVersionNumber,
                isEdit: data.isEdit!=='N'
            })
        })
    }
                ...
                render: (text, record) => (
                    <div style={{textAlign: 'center'}}>
                        <Tooltip title={HAP.languageChange("lookup.delete")} placement="bottom" getTooltipContainer={(that) => that}>
                            <a className="operateIcon small-tooltip" onClick={this.handleOpen.bind(this, record.lookupValueId)} disabled={!this.state.isEdit}>
                                <Icon type="delete" style={{color: !this.state.isEdit ? "#B9C0C8" : "#08ABF4"}}/>
                            </a>
                        </Tooltip>
                        <Tooltip title={HAP.languageChange("lookup.edit")} placement="bottom" getTooltipContainer={(that) => that}>
                            <a className="operateIcon small-tooltip" onClick={this.showModal.bind(this, record.lookupValueId)} disabled={!this.state.isEdit}>
                                <Icon type="edit" style={{color: !this.state.isEdit ? "#B9C0C8" : "#08ABF4"}}/>
                            </a>
                        </Tooltip>
                    </div>
                )
...
//}}}
        ...
        let initCode = this.state.code ? this.state.code : "";
        let initDesc = this.state.description ? this.state.description : "";
        return (
            <div>
                <Remove open={this.state.open} handleCancel={this.handleClose} handleConfirm={this.handleDelete.bind(this)}/>
                <PageHeader title={HAP.languageChange("lookup.editCode")} backPath="/iam/lookup"/>
                    ...
                    <Form onSubmit={this.handleSubmit.bind(this)}>
                        ...
                        >
                            {getFieldDecorator('typeCode', {
                                ...
                                <Input size="default" disabled={!this.state.isEdit}/>
                              )
                            }
                        </Form.Item>
                        <Form.Item
                        ...
                        >
                           {getFieldDecorator('desc', {
                            ...
                            {getFieldDecorator('lookupValueList')(
                                <div>
                                    <div style={{marginBottom: "10px"}}>
                                        <Button type="primary" onClick={() => {this.showModal()}} disabled={!this.state.isEdit}>
                                    ...
                                    <Modal
                                        ...
                                        footer={[
                                            ...
                                            <Button key="submit" type="primary" size="large" loading={this.state.loading} onClick={this.handleOk.bind(this)} >
                                                {HAP.languageChange("lookup.save")}
                                            </Button>,
                                        ]}
                                    >
                            ...
                            <Form.Item wrapperCol={{offset: 3}}>
                                <Row>
                                    <Col span={2}>
                                        <Button size="default" loading={this.state.submiting} type="primary" htmlType="submit" className="login-form-button" disabled={!this.state.isEdit}>
                                            {HAP.languageChange("lookup.save")}
                                        </Button>
                                    </Col>
                                    <Col span={2}>
...
export default Form.create({})(withRouter(EditLookup));
